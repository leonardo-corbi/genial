...
import { ChevronDown, Info, Loader2, PlusIcon } from "lucide-react";
...
import { AtivoSchemaType, ativoSchema } from "@/lib/zodSchema";
import { useTransition } from "react";
import { tryCatch } from "@/hooks/try-catch";
import { createAtivo } from "../actions";
import { useConfetti } from "@/hooks/use-confetti";

export function AtivosForm() {
  const [pending, startTransition] = useTransition();
  const { triggerConfetti } = useConfetti();

  const form = useForm<AtivoSchemaType>({
  ...
  function onSubmit(values: AtivoSchemaType) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(createAtivo(values));
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Ex: Petrobras, Bitcoin, etc." {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="max-w-xs">
                      <strong>Baixo:</strong> Menor volatilidade, retornos mais
                      estáveis
                      <br />
        ...
        <div className="pt-2">
          <Button type="submit" disabled={pending}>
            {pending ? (
              ...
              <>
                Adicionar Ativo <PlusIcon className="ml-1" size={16} />
              </>
            )}
          </Button>
        </div>
